...
ssize_t Buffer::ReadFd(int fd, int* Errno) {
    // 创新点，参考muduo库
    // 通过临时栈区buff，避免了readv的缓冲区溢出问题
    char buff[65535];   // 栈区

    struct iovec iov[2];
    // 分散/聚集IO
    // 用于readv和writev函数，可以一次读取多个缓冲区的数据
    // 其中iov[0]是buffer_中的可写区域，iov[1]是临时栈区buff
    ...
    if(len < 0) {
        *Errno = errno;
        ...
        // 如果buffer写满了，剩余数据会被放到buff中
        // 然后再调用Append函数，将buff中的数据放到buffer中
        // 可能会涉及扩容等操作
